        ...
        m_spInfo->packages([this, &txn](nlohmann::json & rawData)
                {
                    ...
                    input["data"] = m_spNormalizer->normalize("packages",
                    ...
                    catch (const DbSync::unique_constraint_error & ex)
                    {
                        m_logFunction(SYS_LOG_DEBUG_VERBOSE, ex.what());
                    }
                });
        txn.getDeletedRows(callback);
...
void Syscollector::scanHotfixes()
{
    if (m_hotfixes)
    {
        ...
        const auto& hotfixes { m_spInfo->hotfixes() };
...
void Syscollector::scanProcesses()
{
    if (m_processes)
    {
        ...
        m_spInfo->processes([this, &txn](nlohmann::json & data)
                {
                    nlohmann::json input;

                    data["checksum"] = getItemChecksum(data);
